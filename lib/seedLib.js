...
export const seedGroups = async () => {
  ...
  try {
    ...
    await groups.forEach(async (groupJSON, index) => {
      ...
      newGroupCreator.joinedGroups.push({
        group: newGroup._id,
        role: GROUP_MODERATOR,
      });
      newGroup.usersRoles.push({
        user: newGroupCreator._id,
        role: GROUP_MODERATOR,
      });
  ...
  try {
    ...
    await dbGroups.forEach(async (group) => {
      for (let i = 0; i < getRandomNumber(0, 10); i++) {
        ...
        workUsers = workUsers.filter(
          (workUser) => workUser.email !== user?.email
        );
        ...
        if (user) {
          ...
          group.usersRoles.push({ user: createdUser._id, role: GROUP_MEMBER });
...
export const seedReservations = async () => {
  ...
  try {
    ...
    for (const item of dbItems) {
      ...
      const userIds = group.usersRoles.map((user) => user.user);
